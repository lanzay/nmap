...
func (host rawHost) cleanHost() Host {
	output := Host{
		...
		OS{},
		Times{
			Srtt:   host.Times.Srtt,
			RttVar: host.Times.RttVar,
			To:     host.Times.To,
		},
	}
	...
	output.OS = host.OS.cleanOS()
	//for _, os := range host.OS {
	//	output.OS = append(output.OS, os.cleanOS())
	//}
	...
	if len(h.Ports) != 0 {
		...
		for _, port := range h.Ports {
			for _, line := range strings.Split(port.ToString(), "\n") {
				if line != "" {
					device := ""
					if len(port.Service.Name+port.Service.Method) != 0 {
						device = fmt.Sprintf(" (%s, %s, %s)", port.Service.Name, port.Service.Product, port.Service.Servicefp)
					}
					out += fmt.Sprintf("  %s %s\n", line, device)
...
func (h Host) GetNameUser() string {
